...
class ColorPrinter:
    ...
    def init_colors() -> bool:
        if sys.platform == "win32":
            try:
                """Terminal coloring for Windows is written with Windows
                Console API.
                ...
                def err_check(result, func, args) -> tuple:
                    """This function is a helper for the error checking.It is
                    raises an exception when the API call failed."""
                    if not result:
                        raise WinError()
                    return args

                def get_std_handle() -> WINFUNCTYPE:
                    """GetStdHandle retrieves a handle to the specified
                    standard device (standard input, standard output, or
                    standard error)."""
                ...
                def get_console_mode() -> WINFUNCTYPE:
                    """GetConsoleMode retrieves the current input mode of a
                    console's input buffer or the current output mode of a
                ...
                def set_console_mode() -> WINFUNCTYPE:
                    """SetConsoleMode sets the input mode of a console's input
    ...
    def cprint(self, text: str, color: Optional[str] = COLORS["white"], raw_text="", end: str = "\n") -> None:
