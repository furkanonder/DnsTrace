...
class TestDnsTraceIntegration:
    TIMEOUT = 2
    TEST_CASES = [("localhost", "A"), ("google.com", "AAAA")]
    ...
    def _generate_dns_query(self, dns_server: str, domain: str, record_type: str = "A") -> None:
        try:
            subprocess.run(
    ...
    @pytest.mark.parametrize("test_domain, record_type", TEST_CASES)
    def test_dns_query_capture(self, dnstrace_process, dns_server, test_domain, record_type):
        self._generate_dns_query(dns_server, test_domain, record_type)

        # Target patterns to find in output
        query_type_marker = f"query[{record_type}]"
            ...
            if rlist:
                try:
                    ...
                    while "\n" in buffer:
                        ...
                        if query_type_marker in line and domain_marker in line:
                            return
        ...
        if query_type_marker in buffer and domain_marker in buffer:
            return

        pytest.fail(
            f"Query not found\n"
            f"Looking for: {query_type_marker} and {domain_marker}\n"
            f"Output buffer:\n{buffer}\n"
